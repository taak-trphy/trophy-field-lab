# TROPHY FIELD LAB

TROPHYによる、東京湾奥ビッグベイトの個人フィールドアーカイブ。

## Version 1.2.9

- `data.json` を唯一の記録データとして使用
- FIELD / STRATEGY / OPERATION / TACKLE / LOG / LIBRARYを同じデータから表示
- LOGはPATTERN単位でルアー、カラー、テール、モード、リップ、浮力調整、操作、結果を記録
- PC / スマートフォン共通のリキッド・ワンカラム設計
- GitHub Pagesでは全ファイルをリポジトリのルートへアップロード

## What's New

### 2026.09

- 2026.09.25 23 ANTARES DC MD XG LEFT導入（新品・実釣未使用）
- SWIMBAIT PE SMP X8 4号100mは購入済み・未巻き、Avani Shock Leader SMP［NYLON］40lbを登録
- 3台の役割分担、純正ハンドル評価、汽水使用の運用予定、初回実釣の検証項目を追加
- 釣行数と既存リール・ラインのカウンターは維持

- 2026.09.14 勝島運河・鮫洲橋ログ／大量ベイトと魚種未確認のボイル
- ベイト外周・明暗平行コース、斜め引き、6500C一定巻き、V Tail上下方向の検証項目
- エビス期6500Cを巻き用サブ機／経過観察へ更新
- 6500Cキャスト確認履歴を追加。5501CS RocketとMAX POWER PE6のカウンターは維持

- 2026.09.01 平和島運河・昭和島西側／大森東避難橋〈見晴らし橋〉周辺ログ
- 平和島運河・昭和島西側のFIELD KNOWLEDGE追加
- 貧酸素マップと現場の捕食条件を重ねる検証項目
- 5501CS Rocket再整備、G HANDLE再装着後の状態記録
- MAX POWER PE6を新品交換後2釣行、推定残量約95mへ更新

### 2026.08

- 2026.08.25 荒川リバーステーション周辺・葛西橋明暗ログ
- 強流下のK9グライドコントロール検証
- 荒川FIELD KNOWLEDGE更新
- MAX POWER PE6／PEにシュッを使用後・施工後1釣行へ更新
- 5501CS Rocketメンテナンスカウンター更新
- 2026.08.23 相模湖 Day2ログ
- 相模湖の小型バスと40アップの場所差検証
- TiNY KLASHデッドウォーク／CRAMP Tail検証項目
- K9緑鮒、TiNY KLASH 256、CRAMP TailのTACKLE登録
- K9 Magic TroutのLOST記録
- TiNY KLASHリアフックをSP #1へ更新
- 2026.08.24 MAX POWER PE6新品交換／PEにシュッ施工
- 2026.08.20 芝浦ログ
- ライン干渉とベイト外縁の検証項目
- 芝浦・表層ベイトのレンジ切り分け
- 2026.08.19 PEにシュッ再施工
- 2026.08.19 ワンウェイローラークラッチメンテナンス
- 2026.08.18 勝島運河・鮫洲橋ログ
- Mode B Long Slide + End Micro Pause検証項目
- 浮力調整比較とメンテナンスカウンター更新
- 相模湖 Day1
- K9反応パターン取得
- Summer Maintenance v1.0
- LINE LOG更新

## ブランド

**TROPHY**  
*on my way.*
