...
function tackleIntroductionsView(){
  return (data.tackle.introductions||[]).map(item=>`<section class="tackle-introduction">
    <header><p class="eyebrow">NEW TACKLE / ${esc(item.date)}</p><h3>${esc(item.name)}</h3><p>${esc(item.status)} · ${esc(item.handle)}</p></header>
  ...
  return `<details class="archive-entry">
    ...
    <div class="archive-body">
      <dl class="archive-facts">
        ${l.coordinates?`<div><dt>座標</dt><dd>${esc(l.coordinates)}</dd></div>`:''}
