...
function patternList(patterns){if(!patterns||!patterns.length)return '';return `<div class="pattern-list">${patterns.map(p=>`<article class="pattern-item"><div class="pattern-head"><span>${esc(p.label||'PATTERN')}</span><strong>${esc([p.lure,p.color].filter(Boolean).join(' / '))}</strong></div><dl><div><dt>テール</dt><dd>${esc(p.tail||'—')}</dd></div><div><dt>モード</dt><dd>${esc(p.mode||'—')}</dd></div><div><dt>リップ</dt><dd>${esc(p.lip||'—')}</dd></div><div><dt>操作</dt><dd>${esc(p.action||'—')}</dd></div><div class="pattern-reaction"><dt>結果</dt><dd>${esc(p.reaction||'—')}</dd></div></dl></article>`).join('')}</div>`}
