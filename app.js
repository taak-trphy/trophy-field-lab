...
function logArchiveItem(l,index){const tags=[l.tide,l.phase,l.water,l.bait].filter(v=>v&&v!=='記録なし');return `<details class="archive-item archive-item-single"><summary><div class="archive-head"><div class="archive-meta"><span class="archive-index">${String(index+1).padStart(2,'0')}</span><time>${esc(l.dateLabel||l.date)}${l.time&&l.time!=='記録なし'?` / ${esc(l.time)}`:''}</time><span class="archive-place">${esc(l.place)}</span></div><strong>${esc(l.title||l.place)}</strong><div class="archive-result-preview"><span class="log-label">結果</span><b>${esc(l.result)}</b></div><p>${esc(l.conclusion)}</p></div><span class="archive-toggle">＋</span></summary><div class="archive-detail archive-detail-single"><dl><div><dt>潮・タイミング</dt><dd>${esc([l.tide,l.phase].filter(v=>v&&v!=='記録なし').join(' / ')||'記録なし')}</dd></div><div><dt>水質・ベイト</dt><dd>${esc([l.water,l.bait].filter(v=>v&&v!=='記録なし').join(' / ')||'記録なし')}</dd></div><div><dt>ルアー・操作</dt><dd>${esc(l.method||'記録なし')}</dd></div></dl>${tags.length?`<div class="log-tags">${tags.map(t=>`<span class="tag tag-label">${esc(t)}</span>`).join('')}</div>`:''}</div></details>`}
